...
module.exports = (oldUser, newUser) => {
  ...
  if(!user) return;
  const embed = new discord.MessageEmbed().setColor('#2c2f33').setTimestamp();
  ...
  if(newUser.username.includes(ayarlar.tag) && !user.roles.cache.has(ayarlar.ekipRolu)){
      ...
      if(ayarlar.ekipLogKanali && log) log.send(embed.setDescription(`${user} kişisi ismine \`${ayarlar.tag}\` sembolünü alarak <@&${ayarlar.ekipRolu}> ekibimize katıldı!`).setColor("#2c2f33")).catch();
      ...
      if(ayarlar.ekipLogKanali && log) log.send(embed.setDescription(`${user} kişisi isminden \`${ayarlar.tag}\` sembolünü çıkararak <@&${ayarlar.ekipRolu}> ekibimizden ayrıldı!`).setColor("#2c2f33")).catch();
  }
}

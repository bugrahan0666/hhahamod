...
module.exports.execute = async (client, message, args, ayar, emoji) => {
  let embed = new MessageEmbed().setColor('#36393e').setTimestamp();
  ...
  if (args[0] && (args[0].includes('bilgi') || args[0].includes('info'))) {
    ...
    return message.guild.fetchBan(args.slice(1).join(' ')).then(({ user, reason }) => message.channel.send(embed.setDescription(`**Yasaklanan üye** ${user.tag} (${user.id})\n**Yasaklanma sebebi** ${reason ? reason : "Belirtilmemiş!"}`))).catch(err => message.channel.send(embed.setDescription("Belirtilen ID numarasına sahip bir ban bulunamadı!")).then(x => x.delete({timeout: 5000})));
  };
  ...
  if (!victim) {
    ...
    if(kisi) {
      ...
      if(ayar.banLogKanali && client.channels.cache.has(ayar.banLogKanali)) client.channels.cache.get(ayar.banLogKanali).send(new MessageEmbed().setColor('#36393e').setTimestamp().setTitle('Bir kullanıcı yasaklandı!').setDescription(`**Yasaklayan yetkili** ${message.author} (${message.author.id})\n**Yasaklanan kullanıcı** ${kisi.tag} (${kisi.id})\n**Sebep** ${reason}`));
    } else {
  ...
  message.channel.send(embed.setDescription(`\`${victim.user.tag}\` üyesi ${message.author} tarafından **${reason}** nedeniyle **banlandı!**`));
  if(ayar.banLogKanali && client.channels.cache.has(ayar.banLogKanali)) client.channels.cache.get(ayar.banLogKanali).send(new MessageEmbed().setColor('#36393e').setTimestamp().setTitle('Bir kullanıcı yasaklandı!').setDescription(`**Yasaklayan yetkili** ${message.author} (${message.author.id})\n**Yasaklanan kullanıcı** ${victim.user.tag} (${victim.user.id})\n**Sebep** ${reason}`));
};

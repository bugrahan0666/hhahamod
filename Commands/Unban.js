...
module.exports.execute = async (client, message, args, ayar, emoji) => {
  ...
  if(kisi) {
    ...
    if(ayar.banLogKanali && client.channels.cache.has(ayar.banLogKanali)) client.channels.cache.get(ayar.banLogKanali).send(new MessageEmbed().setColor('#2c2f33').setTimestamp().setTitle('Bir yasak kaldırıldı').setDescription(`Yasağı kaldıran yetkili ${message.author} (${message.author.id})\nYasağı kalkan üye ${kisi.tag} (${kisi.id})`));
  } else {

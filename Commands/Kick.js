...
module.exports.execute = async (client, message, args, ayar, emoji) => {
  ...
  if(!ayar.banciRolleri) return message.channel.send(embed.setDescription("Sunucuda herhangi bir 'Yasaklama komutu bulunmuyor' 'Panel' Komutu ile kontrol ediniz!")).then(x => x.delete({timeout: 5000}));
  ...
  if(ayar.banLogKanali && client.channels.cache.has(ayar.banLogKanali)) client.channels.cache.get(ayar.banLogKanali).send(new MessageEmbed().setColor('#2c2f33').setTimestamp().setTitle('Üye Kicklendi!').setDescription(`**Kickleyen Yetkili:** ${message.author} (${message.author.id})\n**Kicklenen Üye:** ${victim.user.tag} (${victim.user.id})\n**Sebep:** ${reason}`));
};

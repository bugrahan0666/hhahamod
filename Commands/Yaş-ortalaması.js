...
module.exports.execute = async(client, message, args, ayar, emoji) => {
  ...
  message.channel.send(embed.setDescription(`**Not** Bu bilgiler manuel olarak ayarlanmamaktadır. Bot tarafından, sunucuda kayıtlı olan üyelerin yaş ortalamaları alınarak gösterilmektedir.`).addField('Anlık sunucu yaş ortalaması', `\`Genel\` ${client.emojiSayi(`${ortalama(genel)}`)}\n\`Erkek\` ${client.emojiSayi(`${ortalama(erkek)}`)}\n\`Kız\` ${client.emojiSayi(`${ortalama(kiz)}`)}\n\`Ekip\` ${client.emojiSayi(`${ortalama(tagli)}`)}\n\`Ses\` ${client.emojiSayi(`${ortalama(ses)}`)}`).setThumbnail(message.guild.iconURL({dynamic: true, size: 2048})));
};

...
module.exports.execute = async (client, message, args) => {
  if(!conf.sahip.includes(message.author.id))
    if((message.guild.ownerID != message.author.id)) return message.channel.send("Panel komutunu görüntülemek için yetkiniz bulunmamaktadır!");
  let ozellikler = [
    { name: "tag", type: "tekil" },
  ...
  let secim = args[0];
  const embed = new MessageEmbed().setColor('#2c2f33').setTimestamp();
  if (secim == "liste") {

...
module.exports.execute = async (client, message, args, ayar, emoji) => {
  ...
  message.channel.send(new MessageEmbed().setColor('#36393e').setTimestamp().setThumbnail(uye.user.avatarURL({dynamic: true, size: 2048})).setDescription(`${emoji("gif1")} Kayıt olan kullanıcı ${uye}\n${emoji("gif1")} Verilen roller ${uye.roles.cache.filter(a => a.name !== "@everyone").map(x => x).join(', ')}\n${emoji("gif1")} Kayıt eden ${message.author}\n${emoji("gif1")} Güncellenen isim \`${yazilacakIsim}\``)).catch();
};
